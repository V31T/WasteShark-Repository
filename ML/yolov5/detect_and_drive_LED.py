...
class LEDController:
    """Controls RGB status LEDs for target detection and battery status"""
    
    def __init__(self, 
                 status_r_pin=29, status_g_pin=36, status_b_pin=37,
                 battery_r_pin=38, battery_g_pin=40, battery_b_pin=37,
                 use_gpio=True):
        # Status LED pins (LED1) - Shows detection/movement state
        self.status_r = status_r_pin
        self.status_g = status_g_pin
        self.status_b = status_b_pin
        
        # Battery LED pins (LED2) - Shows battery level
        self.battery_r = battery_r_pin
        self.battery_g = battery_g_pin
        self.battery_b = battery_b_pin
        ...
        if self.enabled:
            try:
                ...
                GPIO.setup(self.status_r, GPIO.OUT)
                GPIO.setup(self.status_g, GPIO.OUT)
                GPIO.setup(self.status_b, GPIO.OUT)
                
                # Setup battery LED (LED2)
                GPIO.setup(self.battery_r, GPIO.OUT)
                GPIO.setup(self.battery_g, GPIO.OUT)
                GPIO.setup(self.battery_b, GPIO.OUT)
    ...
    def set_status_led(self, target_detected, moving):
        ...
        if self.enabled:
            if target_detected and moving:
                # Green: Detected and moving toward target
                GPIO.output(self.status_r, GPIO.LOW)
                GPIO.output(self.status_g, GPIO.HIGH)
                GPIO.output(self.status_b, GPIO.LOW)
            elif target_detected and not moving:
                # Blue: Target reached (stopped at target)
                GPIO.output(self.status_r, GPIO.LOW)
                GPIO.output(self.status_g, GPIO.LOW)
                GPIO.output(self.status_b, GPIO.HIGH)
            else:
                # Red: No target detected
                GPIO.output(self.status_r, GPIO.HIGH)
                GPIO.output(self.status_g, GPIO.LOW)
                GPIO.output(self.status_b, GPIO.LOW)
        else:
            # Simulation mode
    ...
    def set_battery_led(self, battery_percent):
        ...
        if self.enabled:
            if battery_percent > 50:
                # Green: Good battery
                GPIO.output(self.battery_r, GPIO.LOW)
                GPIO.output(self.battery_g, GPIO.HIGH)
                GPIO.output(self.battery_b, GPIO.LOW)
            elif battery_percent > 20:
                # Yellow: Medium battery (R+G = Yellow)
                GPIO.output(self.battery_r, GPIO.HIGH)
                GPIO.output(self.battery_g, GPIO.HIGH)
                GPIO.output(self.battery_b, GPIO.LOW)
            else:
                # Red: Low battery
                GPIO.output(self.battery_r, GPIO.HIGH)
                GPIO.output(self.battery_g, GPIO.LOW)
                GPIO.output(self.battery_b, GPIO.LOW)
        else:
            # Simulation mode
            if battery_percent > 50:
    ...
    def cleanup(self):
        ...
        if self.enabled:
            try:
                ...
                GPIO.cleanup([
                    self.status_r, self.status_g, self.status_b,
                    self.battery_r, self.battery_g, self.battery_b
                ])
...
class NavigationController:
    ...
    def calculate_control(self, detections, frame_width, frame_height):
        ...
        if not detections:
            ...
            if self.led:
                self.led.set_target_detected(False)
        ...
        # Target found! Turn LED green
        if self.led:
            self.led.set_target_detected(True)
...
def control_thread(rover_controller, nav_controller, led_controller):
    ...
    while not shutdown_flag.is_set():
        if not rover_active.is_set():
            ...
            if led_controller:
                led_controller.set_target_detected(False)
            time.sleep(dt)
            continue
...
def main():
    ...
    # LED arguments
    parser.add_argument("--led-green-pin", type=int, default=17, help="GPIO pin for green LED (BCM numbering)")
    parser.add_argument("--led-red-pin", type=int, default=27, help="GPIO pin for red LED (BCM numbering)")
    parser.add_argument("--no-led", action="store_true", help="Disable LED indicators")
    ...
    led = None
    if not args.no_led:
        led = LEDController(args.led_green_pin, args.led_red_pin, use_gpio=True)
    ...
    if not args.no_rover:
        ...
        if rover.connect():
            ...
            if rover.arm():
                nav = NavigationController(rover, led, args.target_classes)
        ...
        # Create nav controller for LED updates only
        if led:
            nav = NavigationController(RoverController(), led, args.target_classes)
    ...
    if nav is not None:
        control_thread_handle = threading.Thread(
            ...
            args=(rover if rover else RoverController(), nav, led),
            daemon=True
        )
